...
import { ReportInvalidClicksQuery } from './report-invalid-clicks.query';
import { ReportInvalidClicksStore } from './report-invalid-clicks.store';

@Injectable()
export class ReportInvalidClicksService
    extends BaseReportService<ReportInvalidClicksStore, ReportInvalidClicksQuery, ReportInvalidClicksApi>
    implements BaseReportInterface, BaseTransactionListReportServiceInterface
{
    constructor(
        protected store: ReportInvalidClicksStore,
        protected query: ReportInvalidClicksQuery,
        protected api: ReportInvalidClicksApi,
    ...
    index(filters?: Filter2Interface): Observable<StatisticModel[]> {
        return this.api.get(filters, 'reports-clicks-log', 'transactions').pipe(
            tap(({ pagination }) => {
    ...
    getColumnsOptions(): Observable<ConfigTableColumn2Model[]> {
        ...
        return this.api
            .getColumnsOptions('reports-clicks-log-options')
