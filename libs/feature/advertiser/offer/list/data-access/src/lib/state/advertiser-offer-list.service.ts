...
} from '../models/advertiser-offer-list.model';
import { AdvertiserOfferListQuery } from './advertiser-offer-list.query';
import { AdvertiserOfferListState, AdvertiserOfferListStore } from './advertiser-offer-list.store';

@Injectable()
export class AdvertiserOfferListService extends BaseEntityService<AdvertiserOfferListState> {
    constructor(
        private jsonConvertService: JsonConvertService,
        protected store: AdvertiserOfferListStore,
        protected query: AdvertiserOfferListQuery,
        private readonly api: AdvertiserOfferListApi,
    ...
    public list(): Observable<AdvertiserOfferListModel[]> {
        return combineLatest([
            ...
            map(([params]) => AdvertiserOfferListService.transformParamsToOfferQueryParams(params)),
            switchMap((filters) => this.api.index(filters)),
            tap(({ pagination }) => {
                this.updateDataValue({ pagination });
            }),
            pluck('results'),
            tap((offers: AdvertiserOfferListModel[]) => {
    ...
    export(format: SheetExtensionType): Observable<HttpResponse<ArrayBuffer>> {
        const params = AdvertiserOfferListService.transformParamsToOfferQueryParams(this.query.getParams());
